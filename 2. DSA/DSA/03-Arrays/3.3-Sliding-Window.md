...
class Solution {
    public int[] getSubarrayBeauty(int[] nums, int k, int x) {
        ...
        int[] freq = new int[51]; // for -1 to -50
...
public class MinimumWindowSubstring {
    ...
    **Time:** O(m + n), **Space:** O(m + n)
    ...
    // Optimized : with Array and counter
    public static String minWindowOptimized(String s, String t) {
        if (s.length() < t.length()) return "";
        
        // Frequency array for t (ASCII)
        ...
        int matched = 0; // number of characters matched (including duplicates)
        
        for (int right = 0; right < s.length(); right++) {
            char c = s.charAt(right);
            
            // If this character is needed, increment matched
            if (tMap[c] > 0) {
                matched++;
            }
            
            // Decrement count (acts as window adjustment)
            tMap[c]--;
